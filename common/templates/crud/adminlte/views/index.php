...
$nameModel = Inflector::pluralize(Inflector::camel2id(StringHelper::basename($generator->modelClass)));
...
if (($tableSchema = $generator->getTableSchema()) === false) {
    foreach ($generator->getColumnNames() as $name) {
        if (++$count === 1 OR $name === 'is_archived') continue;
        if ($count < 6) {
            echo "    '" . $name . "',\n";
        } else {
            echo "    // '" . $name . "',\n";
    ...
    foreach ($tableSchema->columns as $column) {
        if (++$count === 1 OR $column->name === 'is_archived') continue;
...
<?= "   <?= @themes\custom\widgets\AdminlteBoxGrid::widget([
        ...
        'options' => [
            'class' => 'box-warning',
            'box-title' => '<i class=\"fa fa-bars\"></i> Listado de ".$nameModel."',
