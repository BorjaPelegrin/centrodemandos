...
namespace common\templates\crud;
...
class Generator extends \yii\gii\Generator
{
    ...
    {
        return 'Generador automático con plantilla de Adminlte';
    }
    ...
    public function generate()
    {
        ...
        // Crea el fichero controller
        $files = [
            new CodeFile($controllerFile, $this->render('controller.php')),
        ];

        // Crea el fichero search
        if (!empty($this->searchModelClass)) {
            $searchModel = Yii::getAlias('@' . str_replace('\\', '/', ltrim($this->searchModelClass, '\\') . '.php'));
            $files[] = new CodeFile($searchModel, $this->render('search.php'));
        }

        // Crea los ficheros con subcarpeta de views
        ...
        foreach (scandir($templatePath) as $file) {
            ...
            if (is_dir($templatePath.'/'.$file) && $file != '.' && $file != '..') {
                $files = $this->generateFiles($viewPath,$templatePath,$files,$file);
            }
        }
     ...
     */
    public function generateFiles($viewPath,$newPath,$files,$dir)
    {
        $templatePath = $newPath.'/'.$dir;
        foreach (scandir($templatePath) as $file) {
            if (is_file($templatePath . '/' . $file) && pathinfo($file, PATHINFO_EXTENSION) === 'php') {
                $files[] = new CodeFile("$viewPath/$dir/$file", $this->render("views/$dir/$file"));
            }
        }
